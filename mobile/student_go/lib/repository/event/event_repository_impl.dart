...
class EventRepositoryImpl extends EventRepository {
  ...
  Future<ListEventsResponse> getUpcomingEventsLimited(
    ...
    try {
      final response = await _httpClient.get(
        Uri.parse(
            'http://172.20.10.11:8080/event/upcoming-limit/$city?page=$page&size=$size'),
      );
  ...
  Future<ListEventsResponse> getAccordingEventsLimited(
    ...
    try {
      final response = await _httpClient.get(
        Uri.parse(
            'http://172.20.10.11:8080/event/according-limit/$city?page=$page&size=$size'),
  ...
  Future<ListEventsResponse> getEventsByEventType(
    ...
    try {
      final response = await _httpClient.get(
        Uri.parse(
            'http://172.20.10.11:8080/event/by-event-type/$city/$eventTypeId?page=$page&size=$size'),
        //Uri.parse('http://172.20.10.11:8080/auth/login'),
      );
  ...
  Future<EventDetailsResponse> getEventDetails(String eventId) async {
    try {
      final response = await _httpClient.get(
        Uri.parse('http://172.20.10.11:8080/event/details/$eventId'),
        //Uri.parse('http://172.20.10.11:8080/auth/login'),
      );
  ...
  Future<List<Content>> getAllEvents(String cityName) async {
    try {
      final response = await _httpClient.get(
        Uri.parse('http://172.20.10.11:8080/event/upcoming/$cityName'),
        //Uri.parse('http://172.20.10.11:8080/auth/login'),
      );
  ...
  Future<List<Content>> getUpcomingEventsFiltered(
    ...
    try {
      ...
      final response = await _httpClient.get(
        Uri.parse(
            'http://172.20.10.11:8080/event/upcoming/$city?&eventName=$name&eventTypes=$eventTypeString&start=$startFormatted&end=$endFormatted&min=$minPrice&max=$maxPrice'),
      );
  ...
  Future<Uint8List> getEventPhoto(String eventId, int index) async {
    try {
      final response = await _httpClient.get(
        Uri.parse(
            'http://172.20.10.11:8080/event/download-event-photo/$eventId/number/$index'),
      );
